        ...
        public async Task Join(int applicationId)
        {
            try
            {
                ...
                if (userClaims is null)
                {
                    ...
                    await _chatAppService.JoinUserToChatAsync(userId, applicationId.ToString(), Context.ConnectionId);
                }
                else
                {
                    var userId = Convert.ToInt32(userClaims.FirstOrDefault(claim => claim.Type == "sub").Value);
                    await _chatAppService.JoinAdminToChatAsync(userId, applicationId.ToString(), Context.ConnectionId);
                }
