        ...
        public async Task < List < (CommonTraitType commonTraitType, List < CommonTrait > commonTraits) >> GetNamedCommonTraitsAndTraitTypeByEntityType(EntityType entityType) {
            ...
            if (entityType == EntityType.Housing) {
                ...
                if (entityType == EntityType.School || entityType == EntityType.Course) {
                    ...
                    var traitIds = schoolTraitIds.Concat(courseTraitIds);

                    var response =
                        traitTypes.Select(trait => (commonTraitType: trait, commonTraits: traits.GetValueOrDefault(trait.Id).Where(ct => traitIds.Contains(ct.Id)).ToList())).Where(x => x.commonTraits != null)
                        .ToList();
                    return response;
                } else {
