...
namespace QuartierLatin.Backend.Tests.CourseCatalogTests.CourseTests
{
    public class AdminGetByIdcourseTest : TestBase
    {
        ...
        public async Task Admin_Should_Be_Able_To_Get_course_By_IdAsync(JObject school, int foundationYear, JObject course)
        {
            ...
            var courseEntityResponse = SendAdminRequest<JObject>($"/api/admin/courses/{courseId}", null);

            Assert.Equal(id, courseEntityResponse["schoolId"]);
        }
